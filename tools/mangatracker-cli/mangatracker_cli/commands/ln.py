...
def add_parser(subparsers):
    parser = subparsers.add_parser("ln", help="Veille light novels / web novels JP")
    action_sub = parser.add_subparsers(dest="action", required=True)
    p = action_sub.add_parser("scan-ranking", help="Scan ranking WN/LN")
    p.add_argument("--source", required=True)
    p.add_argument("--format", choices=["markdown", "json"], default="markdown")
    p.add_argument("--output", default="exports")
    p = action_sub.add_parser("scan-label", help="Scan label LN")
    p.add_argument("--source", required=True)
    p.add_argument("--format", choices=["markdown", "json"], default="markdown")
    p.add_argument("--output", default="exports")
    p = action_sub.add_parser("detect-adaptation", help="Détection adaptation manga/anime")
    p.add_argument("--source", required=True)
