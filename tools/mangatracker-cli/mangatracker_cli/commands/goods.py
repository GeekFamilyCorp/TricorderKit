...
def add_parser(subparsers):
    parser = subparsers.add_parser("goods", help="Veille goodies / figurines JP")
    action_sub = parser.add_subparsers(dest="action", required=True)
    p = action_sub.add_parser("scan-maker", help="Scan fabricant")
    p.add_argument("--maker", required=True)
    p.add_argument("--format", choices=["markdown", "json"], default="markdown")
    p.add_argument("--output", default="exports")
    p = action_sub.add_parser("scan-preorder", help="Scan précommandes")
    p.add_argument("--maker", required=True)
    p.add_argument("--format", choices=["markdown", "json"], default="markdown")
    p.add_argument("--output", default="exports")
