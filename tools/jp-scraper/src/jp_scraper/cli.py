...
app = typer.Typer(help="Open source Japanese scraper optimized for low-token agent workflows.")
...
def sources_audit(config: Path = Path("config/sources.yaml")):
...
def scrape_source(
    ...
    config: Path = Path("config/sources.yaml"),
    selectors: Path = Path("config/selectors.yaml"),
    out: Path = Path("runs")
):
