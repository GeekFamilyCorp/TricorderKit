...
- **Contexte** : les cartes de filière (MOC franchises + IX000) ont révélé 4 anomalies systémiques dans le vault de contenu `C:\Users\sebas\Documents\obsidian\Japan-Alliance` (non versionné git → toute opération destructrice = HIGH risk → archivage réversible imposé).
...
- **Routage (DEC-016)** : prompts des tâches planifiées = `C:\Users\sebas\Documents\Claude\Scheduled\` (hors repo versionné) ; cette décision + journalisation → repo **TricorderKit**.
- **Statut** : Appliquée.
