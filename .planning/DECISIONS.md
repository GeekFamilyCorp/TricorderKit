...
- **Probe (read-only, paramiko via tailnet 100.84.70.50)** : CLI `paperclipai` dans le conteneur ; app `/paperclip` ; API REST sous `/api`, auth = **better-auth** (`POST /api/auth/sign-in/email`, admin geekfamilycorp@gmail.com) ; mutations board exigent header **Origin de confiance** (`http://paperclip-cmnr.srv947484.hstgr.cloud`) sinon "Board mutation requires trusted browser origin". Pas de `company create`/`agent create` en CLI : creation par **`POST /api/companies`** et **`POST /api/companies/{id}/agents`** (route agents company-scoped). Import de package portable possible (`/api/companies/import[/preview]`) mais creation REST directe retenue (reversible, incrementale, verifiable).
...
- **Reste (enrolement runtimes - hors session, besoin Sebastien)** : (a) Claude/Codex : `paperclipai agent local-cli <url-key> -C 4241... --api-base https://paperclip-cmnr.srv947484.hstgr.cloud` a lancer **sur le poste** (installe skills ~/.claude, ~/.codex + cle agent) ; (b) Hermes : cabler `adapterConfig`/`env` de l'agent Veille vers l'endpoint Ollama/Hermes interne du VPS ; (c) **Antigravity** : endpoint webhook a fournir (non assigne a un noeud pour l'instant) ; (d) decision d'activation des heartbeats + confirmation budgets.
