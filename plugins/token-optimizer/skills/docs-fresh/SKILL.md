...
name: docs-fresh
description: >
  Injecte la documentation officielle a jour de toute bibliotheque ou framework via le MCP
  Context7 (Upstash), pour eviter les hallucinations d'API et le code obsolete.
  Mots-cles : "use context7", "docs a jour", "verifie la doc officielle", "documentation fraiche",
  "API de X", "signature de X", "comment utiliser [lib]", "la derniere version de [framework]".
---
...
Le serveur MCP Context7 doit etre declare dans `.mcp.json` du plugin. Pour le configurer manuellement :
...
- "Ecris un hook React pour l'infinite scroll avec TanStack Query"
- "Comment configurer RLS dans Supabase ?"
