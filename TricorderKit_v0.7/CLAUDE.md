...
- **Propriétaire** : Sébastien / GeekFamilyCorp
...
Politique des hooks : `docs/05_hooks_policy.md`

Règles essentielles :
- Un hook ne doit **jamais** appeler un autre hook directement
- Un hook est un intercepteur léger (≤ 15 lignes de logique réelle)
- Tout appel externe (Temporal, fichier) dans un `try/except` — fail silently
- Format de log obligatoire : `{hook_name, skill_name, timestamp, status, tokens_used}`

Hooks enregistrés (voir `.claude/settings.json`) :

| Événement | Script | Rôle |
|---|---|---|
| `PreToolUse` | `core/hooks/pre_intent_hook.py` | Qualification intention |
| `PreToolUse` | `core/hooks/pre_execution_hook.py` | Estimation coût + risk guard |
| `PostToolUse` | `core/hooks/post_execution_hook.py` | Validation résultat + quality_score |
| `Stop` | `core/hooks/post_session_hook.py` | Déclenche usageObserverWorkflow |
...
TEMPORAL_ADDRESS        → workflow engine (défaut : localhost:7233)
